import streamlit as st
from PIL import Image
import numpy as np
import cv2

DEMO_IMAGE = 'stand.jpg'

BODY_PARTS = {
    "Nose": 0, "Neck": 1, "RShoulder": 2, "RElbow": 3, "RWrist": 4,
    "LShoulder": 5, "LElbow": 6, "LWrist": 7, "RHip": 8, "RKnee": 9,
    "RAnkle": 10, "LHip": 11, "LKnee": 12, "LAnkle": 13, "REye": 14,
    "LEye": 15, "REar": 16, "LEar": 17, "Background": 18
}

POSE_PAIRS = [
    ["Neck", "RShoulder"], ["Neck", "LShoulder"],
    ["RShoulder", "RElbow"], ["RElbow", "RWrist"],
    ["LShoulder", "LElbow"], ["LElbow", "LWrist"],
    ["Neck", "RHip"], ["RHip", "RKnee"],
    ["RKnee", "RAnkle"], ["Neck", "LHip"],
    ["LHip", "LKnee"], ["LKnee", "LAnkle"],
    ["Neck", "Nose"], ["Nose", "REye"],
    ["REye", "REar"], ["Nose", "LEye"],
    ["LEye", "LEar"]
]

width = 368
height = 368
inWidth = width
inHeight = height

# Load trained model
net = cv2.dnn.readNetFromTensorflow("graph_opt.pb")

# Streamlit UI
st.title("Human Pose Estimation")

st.text(
    "Make sure you upload a clear image with all body parts visible."
)

# Upload image
img_file_buffer = st.file_uploader(
    "Upload an image",
    type=["jpg", "jpeg", "png"]
)

# Load image
if img_file_buffer is not None:
    pil_image = Image.open(img_file_buffer)

    # Convert RGBA -> RGB
    pil_image = pil_image.convert("RGB")

    image = np.array(pil_image)

else:
    pil_image = Image.open(DEMO_IMAGE)

    # Convert demo image also
    pil_image = pil_image.convert("RGB")

    image = np.array(pil_image)

# Show original image
st.subheader('Original Image')

st.image(
    image,
    caption="Original Image",
    width="stretch"
)

# Threshold slider
thres = st.slider(
    'Threshold for detecting key points',
    min_value=0,
    value=20,
    max_value=100,
    step=5
)

thres = thres / 100


@st.cache_data
def poseDetector(frame):

    # Create copy to avoid modifying original image
    frame = frame.copy()

    frameWidth = frame.shape[1]
    frameHeight = frame.shape[0]

    # Convert RGB -> BGR for OpenCV
    frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)

    blob = cv2.dnn.blobFromImage(
        frame,
        1.0,
        (inWidth, inHeight),
        (127.5, 127.5, 127.5),
        swapRB=True,
        crop=False
    )

    net.setInput(blob)

    out = net.forward()
    out = out[:, :19, :, :]

    points = []

    for i in range(len(BODY_PARTS)):

        heatMap = out[0, i, :, :]

        _, conf, _, point = cv2.minMaxLoc(heatMap)

        x = (frameWidth * point[0]) / out.shape[3]
        y = (frameHeight * point[1]) / out.shape[2]

        if conf > thres:
            points.append((int(x), int(y)))
        else:
            points.append(None)

    # Draw skeleton
    for pair in POSE_PAIRS:

        partFrom = pair[0]
        partTo = pair[1]

        idFrom = BODY_PARTS[partFrom]
        idTo = BODY_PARTS[partTo]

        if points[idFrom] and points[idTo]:

            cv2.line(
                frame,
                points[idFrom],
                points[idTo],
                (0, 255, 0),
                3
            )

            cv2.circle(
                frame,
                points[idFrom],
                3,
                (0, 0, 255),
                thickness=-1
            )

            cv2.circle(
                frame,
                points[idTo],
                3,
                (0, 0, 255),
                thickness=-1
            )

    # Convert back BGR -> RGB for Streamlit display
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

    return frame


# Run pose detection
output = poseDetector(image)

# Show output
st.subheader('Positions Estimated')

st.image(
    output,
    caption="Positions Estimated",
    width="stretch"
)